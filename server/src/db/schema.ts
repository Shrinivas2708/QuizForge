...
export const participantInfoEnum = pgEnum("participant_info", [
  "name",
  "email",
]);
...
id: text("id")
  ...
  participantFields: jsonb("participant_fields")
    .$type<string[]>()
    .notNull()
    .default(["name"]),
...
export const chatSessionSourcesTable = pgTable(
...
export const messageTypeEnum = pgEnum("message_type", [
  ...
  "document_upload",
  "quiz_form",
  "processing_complete", // ADD THIS NEW TYPE
  "quiz_interaction", // ADD THIS TOO (for the quiz generation status/result)
  'quiz_generated'
]);
...
export const feedbackTable = pgTable("feedback", {
  ...
  userId: text("user_id")
    .notNull()
    .references(() => usersTable.id, { onDelete: "cascade" }),
  quizId: text("quiz_id").references(() => quizzesTable.id, {
    onDelete: "cascade",
  }),
  messageId: text("message_id").references(() => chatMessagesTable.id, {
    onDelete: "cascade",
  }),
