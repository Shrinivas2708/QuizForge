...
function RouteComponent() {
  ...
  function checkActive(expiresAt: Date) {
    ...
    return current < expireDate // true = active, false = expired
  }
  ...
  function parseUserAgent(ua: string) {
    ...
    // Detect API clients like Postman or curl
      ...
      <Card className="w-full h-fit lg:max-w-sm">
        ...
        <CardContent className="space-y-2">
          ...
          <div className="text-sm flex  justify-between px-2 ">
            ...
            <div>
              <span className="text-foreground">Update at:</span>
        ...
        <ScrollArea className="h-[450px]  ">
          <CardContent className="space-y-2">
            {sessions?.map((v, i) => {
              ...
              const isCurrent = v.token === currentSessionId // get currentToken from auth state
                ...
                >
                  <CardContent className="text-sm space-y-1">
                    <div className="flex gap-2 items-center">
                      {uaInfo.icon}
                      <p className="font-semibold text-base">{uaInfo.device}</p>
                      <Badge
                        variant={'outline'}
                        className={
